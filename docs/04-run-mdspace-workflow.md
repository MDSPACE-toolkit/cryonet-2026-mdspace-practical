...
To start a new analysis, use Files < New. Analysis can be reloaded using Files < Load.
...
We reconstruct a volume from the synthetic dataset. The volume must be reconstructed in the same unit as the PDB structure (Ångströms). Enter the pixel size used during dataset generation (3.6 Å/pixel), then click Start Step.
...
After the first iteration, MDSPACE analyzes the ensemble of fitted structures using principal component analysis. The following iterations use PCA-based refinement. In these iterations, the principal component vectors from the previous ensemble are used to guide MD-based flexible fitting in the next iteration that will use NMMD. For that, select the MD THEN NMMD option. The full list of parameters can mostly be left at their defaults.
...
The run may take several minutes, depending on the selected settings and available computing resources. During this time, we will prepare the next steps in the analysis.
